...
- `pnpm check`、`pnpm test`、`pnpm build` 通过；前端为 `29` 个测试文件、`171` 项测试。
- Rust 全量测试 `96` 项：`94 passed / 0 failed / 2 ignored`；两项真实 PMS 测试默认忽略，
...
- `1280x300` 歌词滚动压力视口实测：主歌词中段中心误差约 `0.04px`，展开播放器约
  ...
  同时覆盖已可见行、离屏行、上下边界、切歌归零后的首次居中和内容不足一屏的情况。
...
1. macOS 修复后人工确认控制中心卡片的真实曲名/歌手/专辑/封面，以及媒体键和 seek；
   代码、运行日志和系统字典契约已经闭环，但本轮禁止主动截图，仍保留一次可见验收。
2. macOS 开发态长期听感：连续专辑 gapless、弱网缓冲提示、耳机/扬声器切换、隐藏窗口
   后持续播放和睡眠恢复。
3. 长期记录 PCM underflow 次数；正常网络应为 0。若频繁出现，先调预载/PCM 容量和
   PMS 连接策略，不要直接换闭源内核。
4. ReplayGain、crossfade、响度扫描和离线下载仍是产品能力扩展，不属于本轮缺陷修复。
