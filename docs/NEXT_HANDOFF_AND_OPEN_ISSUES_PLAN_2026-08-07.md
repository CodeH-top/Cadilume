...
- 开发态仍只允许一条 `pnpm tauri dev` 链；凭证只读
  `~/.cadilume-dev-token`；应用登录态则保存在应用配置目录的用户专属 `credentials.json`。
...
## 7. 已确认待开发：默认输出跟随与播放状态确认

2026-08-18 本机验收发现：Cadilume 进程长时间不退出时，原生 `rodio/cpal` 输出流会
继续绑定进程启动时解析出的系统默认设备。macOS 后续切换系统默认输出后，界面可能仍显示
“暂停”（代表正在播放），但实际进度停在 `0:00`，用户只能通过重启进程恢复。这是播放
生命周期缺陷，不接受把重启作为正常使用步骤。

后续实现要求：

1. 当用户没有选择独立应用输出设备时，原生层必须持续跟随 macOS 当前系统默认输出；
   监听或低频探测默认 `DeviceId` 变化，发现变化后只重建音频流并恢复当前曲目、进度、
   音量、队列和 gapless 预排，不得重启 Cadilume 进程。
2. 当 CoreAudio 输出流报错或设备热插拔导致流失效时，沿用同一套有界自动恢复；恢复失败
   要进入明确的错误态，不能静默显示“暂停”并停在零点。显式选择独立设备时不应被默认设备
   变化覆盖，但所选设备失效后应有清晰回退策略。
3. 前端播放状态必须区分“解码准备完成”和“实际开始输出 PCM”：`native_audio_load`
   返回后，自动播放仍保持加载/缓冲态；只有原生输出回调实际消耗首批 PCM、收到带有效进度
   的启动事件后，才把按钮切成“暂停/播放中”。在此之前不得让用户误以为已经开始播放。
4. 加载、输出切换和自动恢复都要覆盖成功、欠载、超时、读取失败、暂停恢复、seek、切歌、
   隐藏窗口和睡眠唤醒；前端旧代事件不能把新曲目从加载态错误改成播放态。

验收门槛：

- 播放中切换 macOS 默认输出，Cadilume 进程保持不变，当前曲目在有界时间内继续播放，
  进度单调推进且不回到 `0:00`。
- 播放按钮触发后，在首批 PCM 真正输出前一直显示加载/缓冲；模拟无音源、断流或输出流
  失败时不得显示“暂停”，而应保持加载并最终进入可重试错误态。
- 自动化覆盖默认设备变化、显式设备锁定、输出流恢复、首 PCM 启动事件和旧代事件隔离；
  通过 `pnpm check`、`pnpm test`、`pnpm build`、Rust 测试及 macOS 实机长驻进程验收后，
  才从本节移入已解决记录。
