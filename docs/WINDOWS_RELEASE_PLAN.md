...
- NSIS 已启用 `English` / `SimpChinese` 与语言选择器，updater 归档使用与 macOS 相同的 minisign 信任链。
...
2. 已完成：`windows-latest` 先执行完整门禁，再构建 release NSIS 与 `.nsis.zip` / `.sig` updater 资产。
...
5. 待完成：真机覆盖自定义标题栏 / DPI / Snap、中文和英文系统语言、语言选择器、全新安装、覆盖升级、卸载、播放中更新、安装完成后的恢复与 Credential Manager 保留行为。
...
- 当前统一 GitHub Release 包含 DMG、macOS updater 与签名、NSIS EXE、Windows updater 与签名，以及唯一的跨平台 `latest.json`。
