...
- [ ] 启动后不显示原生 Windows 标题栏；Cadilume 自定义标题栏左侧品牌居中且尺寸清晰，右侧最小化、最大化/还原和关闭按钮可用。
...
| 应用登录状态异常 | Release 在 Windows 凭据管理器中检查或删除 Cadilume 凭据后重新登录；Debug 与真实 PMS 回归测试检查 `%USERPROFILE%\.cadilume-dev-token` |
