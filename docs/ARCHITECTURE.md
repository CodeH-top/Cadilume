...
- 封面缓存与音频缓存、票据注册表彼此独立；封面按服务器、专辑与尺寸变体去重，缓存内容携带 PMS 源路径指纹，封面修订变化时原位覆盖同一专辑文件。总大小限制为 512 MiB、不限制文件数量，单张封面仍限制 12 MiB。
...
- 启动线程只构造内存状态、loopback 监听器和窗口壳。Plex/媒体代理网络客户端、CoreAudio
  输出流、Now Playing、菜单栏图标和 Dock 自定义图标均延后；恢复暂停队列不会创建原生
  音频引擎，只有明确开始播放后才允许预缓冲下一首。
- React 主壳与资料库页面分包；资料库路由通过动态 `import()` 加载。进入主界面前先完成
  初始化快照、账号信息和最多 24 张首屏封面的有界预热/解码，期间继续显示全窗启动页。
  封面先由 Rust 校验账号与 PMS ACL，再读取磁盘缓存；同一路径的卡片、96px 小黑胶和
  512px 系统媒体封面共用一个 loopback ticket，不因显示尺寸不同重复下载。
- loopback 封面 URL 只存在于当前进程内存，不写入资料库 SQLite 快照。重启后重新签发
  ticket，但对应图片字节继续从 Rust 磁盘缓存读取；无效、过期或失败的 ticket 会被逐出
  前端映射并按有界次数重试。
