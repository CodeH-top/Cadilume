...
- 长列表的边缘自动滚动不能只依赖 `pointermove`：指针停在边缘时不会继续产生移动事件。拖动激活后应
  维持单一 `requestAnimationFrame` 循环，每帧按当前指针位置滚动，并在滚动后重新读取行坐标、更新
  落点和独立定位线；速度按实际帧间隔相对 60Hz 归一，避免高刷屏过快。pointer up/cancel、lost
  capture、Escape 与组件卸载必须同时取消动画帧，防止隐形循环和残留浮层。

## 2026-08-10 — 认证完成与主界面挂载应是一个初始化事务

- 如果认证成功后立即挂载主框架，再分别由侧栏、首页和资料库 effect 拉取服务器、分区、歌单、歌手与
  推荐数据，用户会先看到全面空白，随后内容分批跳入；即使每个请求本身正确，这仍是错误的启动体验。
- 应在启动页或登录页保留期间统一完成首屏请求，生成一个 `InitialLibraryData` 快照后再一次性设置认证
  session 并挂载主界面；主框架和首页以该快照初始化，并跳过首挂载的重复请求。请求失败继续走登录错误
  或启动重试，无服务器/无音乐分区则作为已经加载完成的显式空状态，而不是无限等待。

## 2026-08-10 — 队列事件必须绑定重复曲目的具体实例

- `ratingKey` 只能标识歌曲，不能标识同一歌曲在播放队列中的第几次出现。若预排、无缝交接、封面
  更新或播放错误只比对 `ratingKey + index`，队列插入/删除或重复歌曲会让旧事件误命中新实例，
  造成错误交接或无缘由的播放失败弹窗。
- 每次入队生成唯一 `queueInstanceId`，旧快照恢复时补齐或修复重复 ID；WebView 与 Rust 队列都传递
  `occurrenceId`，预排重排、交接和错误事件在生效前同时校验实例 ID。运行中 reader 失败应先刷新
  本机票据、从当前受限进度静默加载一次，再走质量回退，不能第一次瞬时中断就打断用户。

## 2026-08-10 — 歌单顺序的身份、服务端写入与键盘焦点要一起处理

- PMS 歌单可包含重复歌曲，排序身份必须使用 `/playlists/{id}/items` 返回的 `playlistItemID`；
  `ratingKey` 只能供浏览器演示回退。移动到首位省略 `after`，其余位置传移动后前一项的
  `playlistItemID`，并在 Rust/Tauri 边界校验标识后调用 PMS move endpoint。
- 可写普通歌单禁用列排序并提供拖拽/方向键排序；智能或只读歌单反向保留列排序且不提供拖拽。
  乐观更新失败后优先重读 PMS。方向键触发异步保存时手柄会短暂 disabled，浏览器会把焦点移到
  `body`；应按 `playlistItemID` 记录待恢复项，等 busy 真正解除后再聚焦移动后的手柄。

## 2026-08-09 — 通知实体背景与遮罩阴影是两件事，缓存操作必须按所有权拆分

- 用户要求恢复通知背景时，只应恢复卡片自身的主题面板背景和边框；透明背景叠加大范围
  `box-shadow` / `backdrop-filter` 会在连续通知之间形成整条灰色带，看起来像额外遮罩。
  平铺通知队列应让队列容器保持透明、卡片使用不透明 `var(--panel)`，并移除外扩阴影和模糊。
- 设置页缓存信息应复用播放设置的 `settings-stack + field-row` 层级：左侧名称与单一大小，
  右侧稳定宽度操作。封面清理只撤销封面票据、删除封面磁盘文件并清空前端封面映射；音频
  清理才停止原生播放链并删除分段缓存。两个操作不能继续共享按钮或相互删除对方的缓存。
...
- 仅在每个网络 chunk 入口检查取消位不足以保证缓存一致性；最后一个 chunk 后的 pacing
  等待仍可能在切歌或账号清理时继续运行，随后把已取消的 `.part` rename 成完整文件。
- 限速等待应使用短片段可中断 sleep，并在完整性校验、flush 后和原子提交前再次检查取消位；
  以确定性测试覆盖“最后 chunk 后取消”且断言 `.part` 与 `.audio` 均不存在。

## 2026-08-08 — 渐进容器探测必须隔离、可取消并有界

- 仅收到头部预载并不代表 `Decoder::builder().build()` 一定只读头部；RIFF/MP4 等容器可能
  继续向下载前沿探测元数据。把 builder、时长探测和首批 PCM worker 准备放在 Tauri async
  worker 会让单个质量档永久占住播放请求，前端也无法触发兼容质量回退。
- 正确边界是 `spawn_blocking` + 每质量档准备超时；超时路径必须同时设置下载取消位、唤醒
  `ProgressiveFile` 的条件变量、清理 `.audio.part` 并有界等待任务。仅 `JoinHandle::abort`
  不能中止已经运行的 blocking 线程，所以 reader 本身必须有取消谓词。
- 活动下载状态要把进度对象、临时路径和任务句柄绑定，并按 `Arc::ptr_eq` 做代际匹配；任务
  完成后释放槽位，避免旧代清理误杀新下载。探测和原子改名并发时，若 `.part` 已消失且
  `finished` 已置位，应读取已提交的完整缓存而不是误报媒体不可用。
...
- Plexamp 4.13.2 的公开包清单包含 `blurhash 2.0.5`、`color 4.2.3`、`tinygradient 1.1.5`
  和 linear-gradient 相关包，但包存在本身不能证明其参与 Ultrablur。实现时只采用公开
  `blurhash` 的解码/校验 API，并依据视觉行为独立实现主题算法；不要搬运 ASAR 中的压缩源码、
  私有模块或素材，也不要仅凭依赖名称臆测其内部调用链。
- 真实封面先缩放到 `150×150` canvas，再从四个象限分别生成候选色；综合 HSV/HSL 饱和度、
  亮度、像素权重与候选间距离选择四角颜色，随后做亮度归一和近色拉伸。相比单一主色，这能保留
  封面的冷暖与明暗关系，又不会退化成模糊图片。四个角仍应分别输出，即使原图色彩接近也需保留
  最低可感知差异。
- BlurHash 是预览/失败回退，不是真实封面取色的替代品。PMS 元数据请求统一包含
  `includeFields=thumbBlurHash`，只有通过公开校验器的值才进入播放会话；优先级固定为真实封面、
  当前 BlurHash、上一组有效四色、默认主题。同一封面复用主题，不同封面用约 `420ms` 的类型化
  CSS 颜色插值，封面最终失败时旧主题最多保留 4 秒，避免永久显示上一张专辑的颜色。
- 四角颜色应作为展开播放器根节点 CSS 变量，由根背景统一混合；标题、视觉、歌词与底部控制区
  自身保持透明。浅色和深色模式必须分别派生足够对比度的前景、次要文字和控件色，使歌词不会因
  与某个局部背景靠色而消失。封面组件按曲目重新挂载，防止上一首的慢速加载结果污染当前主题。

## 2026-08-11 — 原生播放等待式停止必须覆盖 detached 缓存任务

- `SegmentReader` 释放不代表无 Range 连续下载已经退出：该下载在缓存 Tokio runtime 上独立
  spawn，仍可能持有文件并在取消收尾时截断部分缓存。reader 与连续下载必须分别持有 RAII 活动
  guard，只有两者都析构后缓存活动数才归零。
- 面向 UI 的普通 Stop 继续只取消并立即返回；清缓存、输出设备切换、队列重建和快速重载属于
  资源生命周期边界，必须等待本次取消的 current/pending decoder worker 退出且共享缓存活动数
  归零。等待需有界并传播超时错误，不能在超时后继续删除或复用缓存文件。
- 快速重载同一缓存身份也要先经过等待屏障，再打开新 reader；否则旧连续下载的取消清理可能在
  新 reader 建立后执行 `set_len(0)`，破坏新一代播放。
- 连续下载测试中的服务端 `ready` 只证明响应已发送，可能早于客户端完成索引提交和后台 spawn；
  断言第二个活动 guard 前应等待客户端活动数达到 2，并保留明确超时以区分同步竞态与真实漏计数。
