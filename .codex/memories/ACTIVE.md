...
- [ACT-021] 开发态（debug）凭证只读写 `~/.cadilume-dev-token`（600 权限，git 忽略），
  ...
  开发态退出把 token 文件不存在视为已退出；其他删除错误必须在清空内存登录态前返回，不能让残留
  凭据在重启后恢复出一个前端已认为退出的会话。
...
- [ACT-032] 已认证启动与新登录都必须在初始化页或登录页内完成首屏资料快照加载，再一次性挂载主界面；
  快照至少包含服务器、音乐分区、歌单、完整歌手目录、首页推荐和最近加入专辑。不得先显示空侧栏或
  空内容区再逐项补数据；请求失败留在当前初始化流程并提供重试，无服务器或无音乐分区则进入明确空状态。
  启动快照只对首次选中的服务器/资料库作用域有效；一旦离开某个作用域就永久失效，切回原标识也必须
  重新请求，不能把其他服务器或资料库的数据与当前 PMS 标识组合使用。
