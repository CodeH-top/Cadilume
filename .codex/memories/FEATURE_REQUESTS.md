...
- 下一个实施项依序为 R8 全局通知队列与三层堆叠、R9 暗色可见性 / 主题封面稳定 / 页面标题统一、R10 展开播放器机械唱臂 / 统一浮层 / 音量收口、R11 原生状态图标与统一最小化、R12 资料库密度 / 时长列 / 多歌手保真；五项完成后才是必须单独立项的 Plex Companion controller / receiver（L1）和 Emby / Jellyfin 实际认证、浏览、播放与歌词接入（L2）。现有配色、Logo 或 adapter 接口均不代表这些服务已支持。
