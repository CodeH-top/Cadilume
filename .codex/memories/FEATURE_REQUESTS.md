...
- 目标：在用户频繁跳歌、seek 或低磁盘环境中，只保存真实读取过的媒体区间，减少完整曲目
  下载造成的带宽与磁盘浪费；它仍属于临时流式缓存，不等同于离线下载。
- 参考事实：本机网易云音乐通过具有完整逻辑长度的稀疏 `.uc!` 文件和 `.idx!` `zone` 区间
  索引，只为已下载块分配磁盘；Plexamp 4.12.0 的公开说明也表明成熟流式缓存可有多个 read
  head。仅借鉴行为原则，不复制第三方源码、私有协议或文件格式。
- 实现门禁：PMS Range 请求与缺口调度、区间合并/持久化索引、缺口感知 Read+Seek、当前
  解码与 seek 的优先级、多个 read head 去重、崩溃恢复、完整性校验，以及按实际分配块参与
  LRU/容量预算。完成前保持当前连续前缀 + 完整文件 LRU，禁止用稀疏零洞冒充已下载数据。
- 离线下载须另立产品能力：用户显式选择、展示预计大小和进度、可暂停/删除，并采用独立容量
  与保留规则，不能由流式缓存悄悄升级成整库副本。
...
- 代码链路已闭环：禁用 WebKit MediaSession；Rust 在主线程用 MediaPlayer 框架导出的真实
  ...
  线程。
- 2026-08-08 已在唯一 Tauri 开发链与真实 PMS 播放中完成可见验收：控制中心显示真实曲名、
  歌手、专辑、封面、时间轴和播放状态，上一首、暂停/播放、下一首均可用；控制中心操作可
  同步 Cadilume，干净重启后一次下一首只触发一次系统命令和一次加载。该项关闭。
