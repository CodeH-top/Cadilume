...
- 先内部浏览器完成受控 UI / DOM 验收；R8–R10 全部完成后才复用唯一 Tauri 开发链做真实统一验收，不截图、不启动第二条链。
...
- 下一个实施项依序为 R8 全局通知队列与三层堆叠、R9 暗色可见性 / 主题封面稳定 / 页面标题统一、R10 展开播放器机械唱臂 / 统一浮层 / 音量收口；三项完成后才是必须单独立项的 Plex Companion controller / receiver（L1）和 Emby / Jellyfin 实际认证、浏览、播放与歌词接入（L2）。现有配色、Logo 或 adapter 接口均不代表这些服务已支持。
