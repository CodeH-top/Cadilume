...
- 已按用户确认的“固定 1 GiB + 稀疏分段 v2”完成：首段 256 KiB、后续对齐 2 MiB Range、
  合并区间索引、缺口感知 Read+Seek、当前曲目优先、真实下一首第二 read head、APFS 显式
  `F_PUNCHHOLE`、实际分配块预算、1 GiB 低磁盘保留和整条目 LRU。
- `206` 区间/长度、ETag/Last-Modified、`416`、崩溃恢复、截断响应、取消和缓存复用均有
  确定性测试；下一首遇到无 Range 的 `200` 不完整 fallback，只有当前曲目保留连续兼容读取。
  旧 `downloads` 缓存和 1–10 GiB 设置已移除。
- 该缓存仍是临时流式缓存，不扫描曲库或整条队列。离线下载继续另立产品能力：用户显式选择、
  展示预计大小和进度、可暂停/删除，并采用独立容量与保留规则，不能由流式缓存升级成整库副本。
